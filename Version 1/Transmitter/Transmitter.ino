...
//    }
 ...
 // }
  ...
  for(int number = 0; number < u8g2.getWidth(); number = number + 2)
  {
    u8g2.clearBuffer();
    u8g2.drawStr(number, u8g2.getHeight() /2, "OR");
    u8g2.sendBuffer();
    delay (0.05);
...
void menuChange(const char *itemName, int value) // This gets called when a menu item is selected or a parameter is changed
{
  ...
  if((strcmp("Get Going",itemName)==0) && isCalibrated())
  {
...
void setup()
{
  ...
  mainMenu->addItem("Main Menu","Get Going");
