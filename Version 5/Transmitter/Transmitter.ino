...
// for(int start=1;start<=70;start+=1)
//{
// u8g2.clearBuffer();
// u8g2.drawEllipse(64,32,u8g2.getWidth()-start*2,u8g2.getHeight() -start);
// u8g2.sendBuffer();
// delay(50);
//}
 for(int start=1;start<=55;start+=1)
{
 u8g2.clearBuffer();
 u8g2.drawFrame(1,1,u8g2.getWidth()-start*2,u8g2.getHeight() -start);
 u8g2.sendBuffer();
 delay(5);
}
 u8g2.drawStr(1,10,"Timmy");
 u8g2.sendBuffer();
 delay(3000);
}
...
void menuChange(const char *itemName, int value) // This gets called when a menu item is selected or a parameter is changed
{
  ...
  if((strcmp("Start!",itemName)==0) && isCalibrated())
  {
...
void setup()
{
  ...
  mainMenu->addItem("Main Menu","Start!");
