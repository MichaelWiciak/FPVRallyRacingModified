// Simple menu system using the OLED (screen) and rotary encoder.  See main code ('Transmitter') for how it's used
...
void knobTurn(bool isTurningClockwise, bool isTurningAntiClockwise)
{
    int encoderValue = R.read();

    if(encoderValue!=0)
  {
    if(encoderValue==DIR_CW) // Going clockwise ...
    {
        isTurningClockwise = true;
        isTurningAntiClockwise = false;
    }
    else // must be counter-clockwise
    {
      isTurningClockwise = false;
      isTurningAntiClockwise = true;
    }
  }
  if(encoderValue=0)
  {
...
void menuService() // must be called regularly to check the encoder, process the menu system, and update the LCD
{
  int encoderValue = R.read();
  
  if(encoderValue!=0)
  {
    if(currentMenuItem==topMenu)
      currentMenuItem=topMenu->childItem;
    else if(encoderValue==DIR_CW) // Going clockwise ...
    {
      if(changingParams) // Allow parameter editing ...
      {
        if(currentMenuItem->defaultVar<currentMenuItem->maxVar)
          currentMenuItem->defaultVar+=currentMenuItem->incVar;
      }
      else if(currentMenuItem->nextItem!=NULL) // ... or allow menu selection
    ...
    else // must be counter-clockwise
    {
      ...
      else if(currentMenuItem->prevItem!=NULL)
